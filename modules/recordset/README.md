...
| <a name="input_timeouts"></a> [timeouts](#input\_timeouts) | Timeout settings for create/update/delete operations on the DNS record set | <pre>object({<br/>    create = optional(string)<br/>    update = optional(string)<br/>    delete = optional(string)<br/>  })</pre> | `null` | no |
| <a name="input_ttl"></a> [ttl](#input\_ttl) | The time-to-live (TTL) value for this record set in seconds. Controls how long DNS resolvers cache this record. | `number` | `null` | no |
